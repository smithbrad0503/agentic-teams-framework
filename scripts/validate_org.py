...
4. Put a provenance header inside the first 12 lines, declaring which of the two
   kinds of file this is.                                   -> check_provenance
   - **Library-sourced** — `agentic-org: v<x.y.z> source=<plugin-relative path>`.
     It has an upstream, so /org-update can diff it against the library.
   - **Project-owned** — `agentic-org: project-owned`. AGENTS ONLY. /org-init
     generated this one from evidence in this repo because the library has no
     identity for it (an integration specialist: stripe-expert, supabase-expert,
     kafka-expert…). There is no upstream file, so there is nothing to diff and
     /org-update never syncs or overwrites it. The marker is explicit on
     purpose: a file with NO header is indistinguishable from one whose header
     was forgotten, so absence is never read as "project-owned".
   Steps 1-3 and 5-6 apply identically to both kinds — only the sync
...
# The second, distinct provenance kind. A project-owned agent was GENERATED by /org-init
# from evidence in the project's own repo (an integration specialist the library has no
# identity for), so it has no `source=` — there is no upstream file to point at and nothing
# for /org-update to diff. Recognising it by an explicit marker, rather than by the absence
# of a header, is deliberate: an absent header is indistinguishable from a bug.
PROJECT_OWNED_RE = re.compile(r"agentic-org: project-owned\b")
...
def check_provenance(path: Path, errs: list[str], *, allow_project_owned: bool = False) -> None:
    """Require one of the recognised provenance kinds in the file's first 12 lines.

    `allow_project_owned` is set only for agent files: an agent is the one thing /org-init
    may generate rather than copy, so it is the one file that may legitimately have no
    upstream `source=`. Every other materialized file came from a library template and must
    say which one, or /org-update cannot reason about it at all.
    """
    head = "\n".join(path.read_text().splitlines()[:12])
    if PROVENANCE_RE.search(head):
        return
    if allow_project_owned and PROJECT_OWNED_RE.search(head):
        return
    if allow_project_owned:
        errs.append(
            f"{path}: missing agentic-org provenance header in the first 12 lines — use "
            "'agentic-org: v<x.y.z> source=<plugin-relative path>' for an agent copied from the "
            "library, or 'agentic-org: project-owned' for one /org-init generated for this repo "
            "(no upstream, never synced by /org-update)"
        )
    else:
    ...
    if not teams:
        ...
        if agents_dir.is_dir():
            ...
            for agent_path in sorted(agents_dir.rglob("*.md")):
                ...
                # Agents are the only files that may be project-owned (generated, no upstream).
                # Every other check above and below applies to them unchanged.
                check_provenance(agent_path, errs, allow_project_owned=True)
